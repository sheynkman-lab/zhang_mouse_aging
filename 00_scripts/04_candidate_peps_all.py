...
# Initialize an empty list to store all peptides from all sheets
all_peptides = []
...
for sheet_name in xls.sheet_names:
    ...
    df['tissue'] = sheet_name
    
    # Append all peptides to the list
    all_peptides.append(df)

# Concatenate all peptides into a single DataFrame
all_peptides_df = pd.concat(all_peptides, ignore_index=True)

# Check the columns of all_peptides_df
print("Peptide Data Columns:", all_peptides_df.columns)
...
# Assuming 'age_effect' should be replaced with the correct column name from all_peptides_df
# Determine change in isoform (peptide) expression
if 'age_effect' in all_peptides_df.columns:
    all_peptides_df['isoform_change'] = all_peptides_df['age_effect'].apply(determine_change)
else:
    print("Error: 'age_effect' column not found in all_peptides_df")
    raise KeyError("'age_effect' column not found in all_peptides_df")

# Assuming 'symbol' should be replaced with the correct column name from all_peptides_df
...
if merge_left_on not in all_peptides_df.columns:
...
merged_df = pd.merge(all_peptides_df, gene_expression_df, left_on=merge_left_on, right_on=merge_right_on)
...
# Output the results
output_file_path = '04_candidate_peps/gene_peptide_changes_all.csv'
