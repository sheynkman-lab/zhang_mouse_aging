...
# Read the CSV file
data <- read_csv("../output/06_isoform_annotation_of_experi_peptides/peptide_to_isoform_mapping_with_experimental_peptides.csv")
...
filtered_data <- data %>%
  ...
  filter(pq31153 == 1 | pq31190 == 1 | pq31193 == 1 | pq31194 == 1) %>% # At least one '1' in the last four columns
  group_by(gene_name) %>% # Group by gene
  filter(n_distinct(transcript_name) >= 2) %>% # Keep genes with two or more unique isoforms
  ungroup()

# Check if the directory exists
output_dir <- "../output/07_filtered_co_expressed_isoforms"
